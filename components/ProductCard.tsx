    ...
    <div className="group flex flex-col gap-6 cursor-pointer relative" onClick={() => onClick(product)}>
      <div className="relative w-full aspect-[4/5] overflow-hidden bg-[#EBE7DE]">
        ...
        <button 
          ...
          className="absolute top-4 right-4 z-10 p-2 rounded-full bg-white/50 backdrop-blur-sm transition-all duration-300 hover:bg-white/80 group/fav"
        >
          <Heart 
            size={18} 
            className={`transition-colors duration-300 ${
              isFavorite ? 'fill-[#2C2A26] text-[#2C2A26]' : 'text-[#2C2A26] group-hover/fav:fill-[#2C2A26]/20'
            }`} 
          />
        </button>
